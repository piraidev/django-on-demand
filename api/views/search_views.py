...
from api.serializers import SupplierProfileSerializer, UserSerializer
import urllib.parse
...
def newest_suppliers(request):
    ...
    """
    supplier_profiles = SupplierProfile.objects.select_related('user').filter(user__first_name__isnull=False, user__last_name__isnull=False, user__details__education__isnull=False, user__details__description__isnull=False, skills__isnull=False).order_by('-date_joined')[:30]
    supplier_serializer = SupplierProfileSerializer(supplier_profiles, many=True)
...
def find_suppliers(request):
    ...
    complete_search_query = search_query_tables + search_query_names_like + search_query_full_text_profile + search_query_full_text_supplier_profile
    results = SupplierProfile.objects.raw(complete_search_query, ['java', 'java', 'java', 'java', decoded_search_term])
